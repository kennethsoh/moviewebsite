...
function showMovieComments(element) {
    ...
    for (var i = 0; i < comment_array.length; i++) {
        if (comment_array[i].movie.trim() === movie_array[item].title.trim()) {
            ...
            star += "<img src='images/edit.png' class='edit' data-toggle='modal' data-target='#editCommentModal' data-dismiss='modal' item='" + i + "' onClick='editComment(this)' />";
...
function addComment() {
    ...
    postComment.send(JSON.stringify(comment)); // Convert the data in Comment object to JSON format 
